...
function Field({ field, isSelected, onSelect, onUpdate }) {
  const [isDragging, setIsDragging] = useState(false);
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 });
  const [isResizing, setIsResizing] = useState(false);
  const [resizeStart, setResizeStart] = useState({ x: 0, y: 0 });
  ...
  const handleMouseMove = useCallback((e) => {
    if (isDragging && fieldRef.current) {
      ...
      const newX = e.clientX - canvasContainer.getBoundingClientRect().left - dragOffset.x;
      const newY = e.clientY - canvasContainer.getBoundingClientRect().top - dragOffset.y;
      
      const canvas = canvasContainer.previousElementSibling;
      const canvasRect = canvas ? canvas.getBoundingClientRect() : canvasContainer.getBoundingClientRect();
      const maxX = canvasRect.width - field.coordinates.width;
      const maxY = canvasRect.height - field.coordinates.height;
